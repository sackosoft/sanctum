...
pub fn main() !void {
    ...
    std.debug.print(
        "Benchmark: {d}k iterations, {d:5.3}ms average (range [{d:5.3}, {d:5.3}]ms). ",
        .{ @divFloor(n, 1000), avg, min_ms, max_ms },
    );
    std.debug.print("LuaJIT - [Setup/Teardown] Always, [Serialization] Always\n", .{});
}
